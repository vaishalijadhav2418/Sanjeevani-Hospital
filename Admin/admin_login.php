...
if (isset($_POST['submit'])) {
    ...
    $select_user = $conn->prepare("SELECT * FROM `admin_users` WHERE LOWER(TRIM(email)) = ?");
    ...
    if ($row) {
        ...
        if ($pass != $rpass) {
            ...
            $insert_user = $conn->prepare("INSERT INTO `admin_users` (name, password, email) VALUES (?, ?, ?)");
...
if (isset($_POST['submit1'])) {
    ...
    if (!empty($email)) {
        $select_user = $conn->prepare("SELECT id, name, password FROM `admin_users` WHERE email = ?");
        $select_user->execute([$email]);
        $row = $select_user->fetch(PDO::FETCH_ASSOC);

        if ($row) {
            if (password_verify($_POST['pass1'], $row['password'])) {
                $_SESSION['admin_id'] = $row['id']; // Set admin_id instead of user_id
                $_SESSION['admin_name'] = $row['name']; // Set admin_name
                echo "<script>
                        Swal.fire({
                            icon: 'success',
